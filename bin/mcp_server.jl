...
function graphs(args, key="source")
    return if haskey(args, key) && args[key] !== nothing
        String.(collect(args[key]))
    else
        String[]
    end
end

function str(args, key, default=nothing)
    return haskey(args, key) && args[key] !== nothing ? String(args[key]) : default
end
...
TOOLS = [
    ...
    MCP.MCPTool(;
        ...
        handler=guarded(
            a -> tool_explain_rule(
                str(a, "rule"); source=graphs(a), limit=get(a, "limit", 25)
            ),
        ),
    ),
    MCP.MCPTool(;
