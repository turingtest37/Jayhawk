...
# PROV-O separates the thing from the making of it. A firing GRAPH is a prov:Entity -- it
# holds triples and was generated at a time -- while the rule application is a
# prov:Activity, and conflating them is why the record could not say who did what with
# which plan. Both are minted per firing, and the activity is reachable from the firing
# through prov:wasGeneratedBy so that `undo_firing!` can find and retract it without
# parsing an IRI apart.
new_activity() = string("urn:jayhawk:activity:", UUIDs.uuid4())

"""
    agent_iri(actor) -> String

The `prov:Agent` for an actor name.

`prov:wasAssociatedWith` needs a resource, not a literal, and actor names arrive from a
caller or an MCP tool call, so the name is percent-encoded rather than interpolated. The
`jayhawk:actor` literal stays alongside it: the IRI is for joining, the literal for reading.
"""
...
function record_firing!(f::Firing; actor::AbstractString, ep::SparqlEndpoint=endpoint())
    ...
    act = new_activity()
    agent = agent_iri(actor)
    # prov:used and prov:wasAssociatedWith carry the typing in their ranges, so the rule and
    # the agent need no `a prov:Entity` / `a prov:Agent` of their own. That is better PROV
    # practice AND it keeps every triple written here rooted at the firing, the activity or
    # the tombstone -- which is what makes undo able to retract the record completely. A
    # shared `<rule> a prov:Plan` would either be orphaned by the first undo or wrongly
    # deleted while other firings still referred to it.
    prov =
        """
     <$(f.graph)> <$(PROV_NS)wasGeneratedBy> <$act> .
     <$act> a <$(PROV_NS)Activity> ;
         <$(PROV_NS)endedAtTime> "$(_now_xsd())"^^<http://www.w3.org/2001/XMLSchema#dateTime> ;
         <$(PROV_NS)used> <$(f.rule)> ;
         <$(PROV_NS)wasAssociatedWith> <$agent> .
 """ * join(
            (
                "    <$(f.graph)> <$(PROV_NS)wasDerivedFrom> <$(check_iri(g))> .\n" *
                "    <$act> <$(PROV_NS)used> <$(check_iri(g))> .\n" for g in f.source
            ),
            "",
        )
    # A tombstone is an entity in its own right: generated by the same activity, derived
    # from the graph it was carved out of. That pair is what makes "what was once true"
    # answerable -- see `retractions`.
    #
    # Deliberately NOT prov:wasInvalidatedBy. That relates an *entity* to the activity that
    # ended it, and the things which ceased to hold here are individual triples, which have
    # no IRIs. Saying it of the tombstone would assert the tombstone ceased to exist, which
    # is the opposite of true: it was just created. Per-triple invalidation would need every
    # retracted triple reified, and the tombstone graph is that record already.
    prov_tomb = isempty(f.tombstone) ? "" : """
        <$(f.tombstone)> a <$(PROV_NS)Entity> , <$(JH_NS)Tombstone> ;
            <$(PROV_NS)wasGeneratedBy> <$act> ;
            <$(PROV_NS)wasDerivedFrom> <$(f.target)> .
    """
...
SELECT ?target ?tomb WHERE { GRAPH <$PROVENANCE_GRAPH> {
  <$g> <$(JH_NS)targetGraph> ?target ; <$(JH_NS)tombstoneGraph> ?tomb } }""";
    ...
    update!("DROP SILENT GRAPH <$g>"; ep=ep)
    # Three subjects, not one. The firing's own triples, the prov:Activity it points at, and
    # -- for a rewrite -- the tombstone's entity record. Retracting only the firing would
    # leave the activity behind as an orphan claiming a rule ran, which is a worse audit
    # trail than none. The activity is found through prov:wasGeneratedBy rather than by
    # taking the IRI apart, so nothing here depends on how it was minted.
    #
    # The rule and the agent keep no triples of their own by design; prov:used and
    # prov:wasAssociatedWith carry their typing in their ranges. So there is nothing shared
    # to orphan and nothing shared to wrongly delete.
    tomb_clause =
        isempty(rw) ? "" : """
...
"""
    update!(
        """
    DELETE { GRAPH <$PROVENANCE_GRAPH> { ?a ?ap ?ao } }
    WHERE  { GRAPH <$PROVENANCE_GRAPH> {
               <$g> <$(PROV_NS)wasGeneratedBy> ?a . ?a ?ap ?ao } } ;
...
export Firing,
    ...
    firings,
    graph_size
export run_rules, retractions, agent_iri, new_activity
