...
""",
    ...
    @testset "gistp:inGraph reads per named graph" begin
        ...
        @testset "what was once true is answerable" begin
            ...
            @testset "PROV-O names the activity, the plan and the agent" begin
                # The 20% interoperability criterion, concretely. prov:used and
                # prov:wasAssociatedWith carry their typing in their ranges, so the rule and the
                # agent need no type assertion of their own -- which is also what keeps every
                # triple rooted at a node undo can reach.
                act = only(select("""
                    SELECT ?a WHERE { GRAPH <$(Jayhawk.PROVENANCE_GRAPH)> {
                      <$(f.graph)> <$(Jayhawk.PROV_NS)wasGeneratedBy> ?a } }"""))["a"]
                a = (act::IRIRef).value
                @test startswith(a, "urn:jayhawk:activity:")
                for (p, o) in (
                    ("$(Jayhawk.PROV_NS)used", "<$(RULES)FlattenIdentifier>"),
                    ("$(Jayhawk.PROV_NS)used", "<$DATA_GRAPH>"),
                    (
                        "$(Jayhawk.PROV_NS)wasAssociatedWith",
                        "<urn:jayhawk:actor:history-test>",
                    ),
                )
                    @test Jayhawk.ask(
                        "ASK { GRAPH <$(Jayhawk.PROVENANCE_GRAPH)> { <$a> <$p> $o } }"
                    )
                end
                @test Jayhawk.ask("""ASK { GRAPH <$(Jayhawk.PROVENANCE_GRAPH)> {
                    <$a> a <$(Jayhawk.PROV_NS)Activity> } }""")
                # the tombstone is an entity generated by the same activity
                @test Jayhawk.ask(
                    """ASK { GRAPH <$(Jayhawk.PROVENANCE_GRAPH)> {
      <$(f.tombstone)> <$(Jayhawk.PROV_NS)wasGeneratedBy> <$a> ;
                       <$(Jayhawk.PROV_NS)wasDerivedFrom> <$DATA_GRAPH> } }"""
                )
            end
