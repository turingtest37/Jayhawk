...
function spec1(;
    ...
    return RuleSpec(
        ...
        "$(R)T_R",
        if match === nothing
            [PatternTriple(iri("$(R)_v"), iri(TYPE), iri("$(HR)In"))]
        else
            match
        end,
        if construct === nothing
            [PatternTriple(iri("$(R)_v"), iri(TYPE), iri("$(HR)Out"))]
        else
            construct
        end,
...
@testset "Jayhawk adversarial suite" begin
    ...
    @testset "1. variableText is untrusted input" begin
        @testset "[PROVEN] a variableText that is not a SPARQL variable is refused" begin
            ...
            @test_throws Exception compile_rule(
                spec1(variables=Dict("$(R)_v" => "?has space"))
            )
        ...
        @testset "[PIN] the same validation as literal-position variables" begin
            ...
            bad = spec1(
                match=[PatternTriple(iri("$(R)_v"), iri("$(HR)p"), var("not a var"))]
            )
    ...
    @testset "2. terms that cannot legally occupy their position" begin
        @testset "[PROVEN] a literal in subject position of R is refused" begin
            ...
            s = spec1(
                construct=[
                    PatternTriple(RDFLiteral("I am a literal"), iri(TYPE), iri("$(R)_v"))
                ],
            )
            @test_throws Exception compile_rule(s)
        end
        ...
        @testset "[PROVEN] a literal in predicate position is refused" begin
            s = spec1(
                match=[
                    PatternTriple(
                        iri("$(R)_v"), RDFLiteral("not a predicate"), iri("$(HR)In")
                    ),
                ],
            )
            @test_throws Exception compile_rule(s)
        ...
        @testset "[PROVEN] blank nodes are not silently reinterpreted across L and R" begin
            ...
            s = spec1(
                match=[
                    PatternTriple(iri("$(R)_v"), iri("$(HR)p"), BNode("b0")),
    ...
    @testset "3. compile_rule's stated contract" begin
        @testset "[PROVEN] a template with no slots: decide, then assert it" begin
            ...
            s = spec1(
                construct=[PatternTriple(iri("$(R)_m"), iri(TYPE), iri("$(HR)Out"))],
                variables=Dict("$(R)_v" => "?v", "$(R)_m" => "?m"),
                mints=Dict(
                    "$(R)_m" =>
                        MintSpec("$(R)_m", "http://ex.org/fixed", Dict{String,RDFTerm}()),
                ),
            )
            @test_throws Exception compile_rule(s)
        end
    ...
    @testset "3b. Rewrite: what the compiler will emit" begin
        ...
        @testset "[PROVEN] a blank node in L∖I cannot be emitted in a DELETE" begin
            ...
            s = rewrite_spec(
                match=[
                    PatternTriple(iri("$(R)_p"), iri("urn:has"), BNode("b0")),
    ...
    if haskey(ENV, "JAYHAWK_TEST_SPARQL")
        ...
        @testset "4a. rules are isolated from one another" begin
            ...
            @testset "[PROVEN] a spec holds only its own rule's variables" begin
                ...
                t = load_rule(TCRULE)
                @test Set(keys(t.variables)) == Set([
                    "http://example.org/tcrules/_a",
                    "http://example.org/tcrules/_b",
        ...
        INSERT DATA {
          ...
          GRAPH <urn:r2:R> { <urn:r2:mint> a <urn:r2:Made> }
                ...
                q2 = try
                    compile_from_store("urn:r2:Rule")
                catch e
                    sprint(showerror, e)
                end
                ...
                qm = try
                    compile_from_store("$(R)PersonToEmployeeRecord")
                catch e
                    sprint(showerror, e)
                end
        ...
        @testset "4b. Assert really computes the least fixpoint" begin
            ...
            engine_result(fs, base) = union(
                Set(
                    ((r["s"]::IRIRef).value, (r["o"]::IRIRef).value) for f in fs for
                    r in select("SELECT ?s ?o WHERE { GRAPH <$(f.graph)> { ?s ?p ?o } }")
                ),
                Set(
                    ((r["s"]::IRIRef).value, (r["o"]::IRIRef).value) for r in select(
                        "SELECT ?s ?o WHERE { GRAPH <$base> { ?s <$(TC)partOf> ?o } }"
                    )
                ),
            )
            ...
            @testset "[FIXED] Assert refuses an empty source rather than under-computing" begin
                ...
                @test isempty(
                    select("""SELECT ?g WHERE { GRAPH ?g { ?s ?p ?o }
            ...
            @testset "[PIN] the closure does not depend on how the data is split across graphs" begin
                ...
                got_a = Set(
                    ((r["s"]::IRIRef).value, (r["o"]::IRIRef).value) for f in a for
                    r in select("SELECT ?s ?o WHERE { GRAPH <$(f.graph)> { ?s ?p ?o } }")
                )
                for f in a
                    undo_firing!(f.graph)
                ...
                got_b = Set(
                    ((r["s"]::IRIRef).value, (r["o"]::IRIRef).value) for f in b for
                    r in select("SELECT ?s ?o WHERE { GRAPH <$(f.graph)> { ?s ?p ?o } }")
                )
                for f in b
                    undo_firing!(f.graph)
        ...
        @testset "4c. a firing means exactly one thing" begin
            ...
            @testset "[PROVEN] apply_rule refuses a non-empty target graph" begin
                ...
                Jayhawk.update!(
                    "INSERT DATA { GRAPH <urn:adv:occupied> { <urn:x> <urn:y> <urn:z> } }"
                )
                @test_throws Exception apply_rule(
                    TCRULE; into="urn:adv:occupied", source=[DATA]
                )
            ...
            @testset "[PROVEN] max_iterations must be a positive integer" begin
                ...
                err = try
                    run_rule(TCRULE; source=[DATA], max_iterations=0)
                catch e
                    e
                end
                @test err isa ArgumentError
        ...
        @testset "4d. the MCP tools are as safe as they claim" begin
            ...
            @testset "[PROVEN] undo_firing refuses a graph that is not a firing" begin
                ...
                @test occursin("not a firing", lowercase(out)) ||
                    occursin("refus", lowercase(out))
            ...
            @testset "[PIN] a hostile source graph IRI is refused, not interpolated" begin
                ...
                @test_throws ArgumentError tool_run_rule(
                    TCRULE;
                    source=[
                        "urn:g> ; DROP ALL ; INSERT DATA { <urn:a> <urn:b> <urn:c> } #"
                    ],
                )
            ...
            @testset "[PIN] dry_run leaves no scratch graph, on every path" begin
                ...
                try
                    tool_explain_rule(TCRULE; source=["urn:adv:missing"])
                catch
                end
                try
                    tool_explain_rule(TCRULE; source=[DATA])
                catch
                end
                @test length(
                    select("SELECT DISTINCT ?g WHERE { GRAPH ?g { ?s ?p ?o } }")
        ...
        @testset "4e. runsparql's transport assumptions" begin
            ...
            @testset "[PIN] a large rule survives the query transport" begin
                ...
                big =
                    "INSERT DATA { GRAPH <urn:adv:big> {" *
                    join(("<urn:s$i> <urn:p> <urn:o$i> ." for i in 1:2000), "\n") *
                    "} }"
                Jayhawk.update!(big)
                q =
                    "SELECT ?s WHERE { GRAPH <urn:adv:big> { ?s <urn:p> ?o } FILTER(?s IN (" *
            ...
            @testset "[PROVEN] Mustache does not eat SPARQL group patterns" begin
                ...
                Jayhawk.update!(
                    "INSERT DATA { GRAPH <urn:adv:m> { <urn:a> <urn:b> <urn:c> } }"
                )
                q = "SELECT ?s WHERE { GRAPH <urn:adv:m> {{ ?s ?p ?o }} }"
                @test length(select(q)) == 1                                    # guard holds
                n = try
                    length(select(q; bindings=Dict("unused" => "x")))
                catch
                    -1
                end
            ...
            @testset "[PIN] an unbound variable is absent, not nothing" begin
                Jayhawk.update!(
                    "INSERT DATA { GRAPH <urn:adv:u> { <urn:a> <urn:b> <urn:c> } }"
                )
                r = only(
                    select("SELECT ?s ?missing WHERE { GRAPH <urn:adv:u> { ?s ?p ?o } }")
                )
        ...
        snapshot(g) = Set(
            string(sparql_text(r["s"]), ' ', sparql_text(r["p"]), ' ', sparql_text(r["o"])) for r in select("SELECT ?s ?p ?o WHERE { GRAPH <$g> { ?s ?p ?o } }")
        )
        ...
        @testset "4f. Rewrite is reversible, exactly" begin
            ...
            @testset "[PROVEN] MCP refuses a bad source arity instead of raising" begin
                ...
                Jayhawk.update!(
                    "INSERT DATA { GRAPH <$TARGET> { <urn:a> <urn:b> <urn:c> } }"
                )
                for src in (String[], [TARGET, "urn:adv:other"])
                    out = try
                        tool_run_rule(RWRULE; source=src)
                    catch e
                        "RAISED"
                    end
                    @test out != "RAISED"
