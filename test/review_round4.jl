...
function enum_spec(;
    values=RDFTerm[RDFLiteral("a"), RDFLiteral("b")],
...
@testset "REVIEW: Round 4 (gistp:oneOf)" begin
    @testset "an enumerated IRI is validated like every other -- FIXED" begin
        ...
        quoted = enum_spec(
            values=RDFTerm[RDFLiteral("a\" } INSERT { <urn:x> <urn:y> <urn:z> } #")]
        )
        ...
        @test failure(
            () -> compile_rule(
                enum_spec(
                    match=[PatternTriple(iri(payload), iri(TYPE), iri("$(G)Person"))]
                ),
            ),
        ) !== nothing
    end
    ...
    @testset "byte-stability does not depend on how the spec was built" begin
        ...
        @test compile_rule(mixed) == compile_rule(
            enum_spec(values=RDFTerm[RDFLiteral("a"), iri("$(EX)b"), iri("$(EX)z")])
        )
    end
    ...
    @testset "broken enumerations" begin
        ...
        @test failure(() -> compile_rule(enum_spec(variables=Dict("$(R)_p" => "?p")))) !==
            nothing
        ...
        @test occursin(
            "contradictory", sprint(showerror, failure(() -> compile_rule(both)))
        )
    ...
    @testset "an enumerated variable used only inside a NAC" begin
        ...
        s = enum_spec(
            construct=[PatternTriple(iri("$(R)_p"), iri("$(EX)tag"), iri("$(EX)fixed"))],
            nacs=[
                NacSpec(
                    "$(R)N",
                    [PatternTriple(iri("$(R)_p"), iri("$(EX)region"), iri("$(R)_c"))],
...
if haskey(ENV, "JAYHAWK_TEST_SPARQL")
    ...
    function load_enum_rule(list_ttl; in_l::Bool)
        scrub()
        lpat = if in_l
            "<$(R)_p> a <$(G)Person> ; <$(EX)region> <$(R)_c> ."
        else
            "<$(R)_p> a <$(G)Person> ."
        end
        Jayhawk.update!("""
    ...
    @testset "REVIEW: Round 4 (store-backed)" begin
        @testset "the coproduct is real: one match becomes N results" begin
            ...
            tags = Set(
                (r["o"]::IRIRef).value for
                r in select("SELECT ?o WHERE { GRAPH <$(f.graph)> { ?s <$(EX)tag> ?o } }")
            )
        ...
        @testset "an empty rdf:List is diagnosed as one -- FIXED" begin
            ...
            @test occursin(
                "never fire", sprint(showerror, failure(() -> compile_rule(spec)))
            )
        end
        ...
        @testset "literal members survive the store round trip with their datatype" begin
            spec = load_enum_rule(
                """( "42"^^<http://www.w3.org/2001/XMLSchema#integer> "42" )"""; in_l=false
            )
