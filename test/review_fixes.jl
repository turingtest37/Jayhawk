...
function spec_with(;
    ...
    return RuleSpec(
        "$(R)Probe", mode, "$(R)Probe_L", "$(R)Probe_R", match, construct, variables, mints
    )
end
...
@testset "REVIEW: fixes for the adversarial findings" begin
    ...
    @testset "1. variableText cannot inject SPARQL" begin
        ...
        @testset "every text-emitting entry point refuses it" begin
            ...
            for e in (
                failure(() -> compile_rule(s)), failure(() -> insert_query(s; into="urn:t"))
            )
        ...
        @testset "the refusal is by whitelist, not by blacklisting the exploit" begin
            ...
            for bad in (
                "person",              # a plausible typo -- no sigil at all
                "?",                   # sigil only
                "?1st",                # leading digit
                ...
                @test failure(
                    () -> compile_rule(spec_with(variables=Dict("$(R)_p" => bad)))
                ) !== nothing
            end
        ...
        @testset "a poisoned template cannot escape its string literal" begin
            ...
            s = spec_with(
                ...
                match=[
                    PatternTriple(iri("$(R)_p"), iri(TYPE), iri("$(G)Person")),
        ...
        @testset "every function that builds SPARQL validates its own inputs" begin
            ...
            for (name, f) in (
                ("check_collisions", () -> check_collisions(s; from=["urn:none"])),
    ...
    @testset "2. undo_firing! is not a general graph delete" begin
        ...
        @test !any(m -> :force in Base.kwarg_decl(m), methods(Jayhawk.tool_undo_firing))
    ...
    @testset "4. run_rule argument validation" begin
        ...
        @test failure(() -> run_rule(s; source=["urn:g"], max_iterations=0)) isa
            ArgumentError
        @test failure(() -> run_rule(s; source=["urn:g"], max_iterations=-1)) isa
            ArgumentError
    ...
    @testset "6. still open (recorded, not asserted as fixed)" begin
        @info "check_iri(\"\")" result =
            failure(() -> check_iri("")) === nothing ? "ACCEPTED" : "refused"
        @info "check_iri(\"not-a-uri\")" result =
            failure(() -> check_iri("not-a-uri")) === nothing ? "ACCEPTED" : "refused"
        # a literal in predicate position is not legal RDF, let alone legal SPARQL
        lit_pred = spec_with(
            match=[PatternTriple(iri("$(R)_p"), RDFLiteral("oops"), iri("$(G)Person"))]
        )
        @info "literal in predicate position" result =
            failure(() -> compile_rule(lit_pred)) === nothing ? "ACCEPTED" : "refused"
        @test true
    end
end # REVIEW
...
if haskey(ENV, "JAYHAWK_TEST_SPARQL")
    ...
    function scrub()
        for f in firings()
            undo_firing!(f.graph; force=true)
        end
        for g in (
            "urn:review:data",
            "urn:review:precious",
        ...
        return Jayhawk.update!("DELETE WHERE { ?s ?p ?o }")
    end
    ...
    @testset "REVIEW (store-backed)" begin
        ...
        @testset "2. undo_firing! refuses what it did not create" begin
            Jayhawk.update!(
                "INSERT DATA { GRAPH <urn:review:precious> { <urn:a> <urn:b> <urn:c> } }"
            )
        ...
        @testset "1. injection is refused on the path that writes to the store" begin
            ...
            s = spec_with(
                variables=Dict(
                    "$(R)_p" => "?x } INSERT { GRAPH <urn:review:pwned> { <urn:a> <urn:b> <urn:c> } } WHERE { ?x a <urn:X>",
                ),
            )
    ...
    INSERT DATA {
      ...
      GRAPH <$(R)B_R> { <$(R)_mB> a <$(HR)Employee> }
            ...
            @testset "and they execute independently" begin
                ...
                mints_a = Set(
                    (r["s"]::IRIRef).value for f in fa for
                    r in select("SELECT ?s WHERE { GRAPH <$(f.graph)> { ?s ?p ?o } }")
                )
                mints_b = Set(
                    (r["s"]::IRIRef).value for f in fb for
                    r in select("SELECT ?s WHERE { GRAPH <$(f.graph)> { ?s ?p ?o } }")
                )
